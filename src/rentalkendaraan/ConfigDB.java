...
public class ConfigDB {
 ...
 private String Database="dbkendaraan";

 public Connection Koneksi;
 
    public ConfigDB(Properties t) { 
                 try {              
...
public Object[][] isiTabel(String SQL, int jumlah){ 
      ...
      Object[][] data=null;        try {           
         ...
         ResultSet rs = st.executeQuery(SQL);          rs.last(); 
         int baris=rs.getRow();          rs.beforeFirst(); 
         int j=0; 
...
public void tampilTabel(String Judul[],String SQL, JTable Tabel){        try { 
         String title[]=Judul;          int jum =title.length; 
...
public void aturLebarKolom(JTable tabel,int baris[]){        try { 
         int getBaris[]=baris;          int JumlahBaris=getBaris.length; 
         tabel.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_OFF);             for (int i = 0; i < JumlahBaris-1; i++) { 
...
public void simpanData(String SQL){      try { 
          Statement st = ConfigDB.this.Koneksi.createStatement();           st.execute(SQL);           st.close(); 
...
public void ubahData(String SQL){      try { 
          Statement st = ConfigDB.this.Koneksi.createStatement();           st.execute(SQL);           st.close(); 
...
public void hapusData(String SQL){      try { 
          Statement st = ConfigDB.this.Koneksi.createStatement();           st.execute(SQL);           st.close(); 
...
public boolean duplikasiData(String tabel,String Key,String nilai){     boolean ada =false; 
    try { 
        Statement st = ConfigDB.this.Koneksi.createStatement(); 
           ResultSet rs  =   st.executeQuery("SELECT*FROM "+tabel+"  WHERE "
                + ""+Key+"="+nilai); 
        if (rs.next()){             ada=true;         }else {ada=false;} 
    } catch (Exception e) { 
...
public void cariData(String Judul[],String Cari, JTable Tabel){     try { 
         Statement st = ConfigDB.this.Koneksi.createStatement();   
          String title[]=Judul;          int jum =title.length;          Tabel.setModel(new DefaultTableModel(isiTabel(Cari,jum), title)); 
    } catch (Exception e) { 
       JOptionPane.showMessageDialog(null,"Maaf Terjadi Kesalahan  pada bagian method cariData : \n ["+e.toString() +"]"); 
    } 
} 

public void tampilLaporan(String laporanFile, String SQL){     try {  
            File file = new File(laporanFile); 
            JasperDesign jasDes = JRXmlLoader.load(file); 
 
            JRDesignQuery sqlQuery = new JRDesignQuery();             sqlQuery.setText(SQL); 
...
public int jumlahRecord(String SQL){         int hasil=0;      int i=0;     try { 
       Statement st=ConfigDB.this.Koneksi.createStatement(); 
    ...
    void aturlebarKolom(JTable JTablerentalkendaraan, int[] lebarKolom) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
